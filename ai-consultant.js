(function () {
  ...
  function extractConsumption(text) {
    ...
    if (/(м2|м²|квадрат|грн|долар|\$|євро)/.test(compact) &&
        !/(квт.?год|квтг|спожив|електроенерг)/.test(compact)) return 0;
    var standalone = /^\s*(\d{2,6})(?:[.,]\d+)?\s*(?:квт(?:·|-)?год)?\s*$/i.exec(source);
    var contextual = /(\d{2,6})(?:[.,]\d+)?\s*(?:квт(?:·|-)?год|квтг)/i.exec(source);
    var described = /(?:спожив\w*|використ\w*|витрача\w*|вироб\w*)[^\d]{0,30}(\d{2,6})/i.exec(source);
    var match = contextual || described || standalone;
    if (!match) return 0;
    var value = Number(match[1]);
    if (/(за\s*(?:рік|річ)|\/рік|на\s*рік|річн)/.test(source)) value = Math.round(value / 12);
    if (/(за\s*(?:день|добу)|\/день|на\s*день|добов)/.test(source)) value = Math.round(value * 30);
  ...
  function recommend(monthly) {
    ...
    message(
      ...
      'Точну комплектацію, монтаж, захист, кабель і конструкцію менеджер розрахує після уточнення умов об’єкта.\n\n' +
  ...
  function answerProductQuestion(text) {
    ...
    if (/^(привіт|добрий день|добрий вечір|вітаю|здраст)/.test(lower)) {
      answer = 'Вітаю! Допоможу зорієнтуватися з типом і потужністю сонячної станції. Напишіть місячне споживання у кВт·год або опишіть об’єкт і що має працювати під час відключень.';
    } else if (/не знаю.*спожив|де.*подив.*спожив|як.*дізнат.*спожив|немає.*квт/.test(lower)) {
      answer =
        'Подивіться споживання у рахунку за електроенергію або в кабінеті постачальника — потрібна кількість кВт·год за місяць. Найкраще надіслати показники за 6–12 місяців, бо взимку й улітку вони відрізняються. Якщо даних немає, перелічіть основні прилади та години їх роботи — менеджер зробить розрахунок навантаження.';
    } else if (/потрібн.*станц|хочу.*станц|станц.*для (будин|дому|бізнес|підприєм|магаз|офіс)/.test(lower) && !/\d{2,6}\s*(?:квт.?год|квтг)/.test(lower)) {
      answer =
        'Підберемо. Для першого розрахунку напишіть споживання електроенергії за місяць у кВт·год. Також важливо: 1 чи 3 фази, дах чи наземна конструкція та чи потрібен резерв під час відключень.';
    } else if (/ніч|без сонц|похмур|взимк|відключенн.*сонц/.test(lower)) {
      answer =
        'Уночі панелі не генерують, а в похмуру погоду виробіток зменшується. Мережева станція бере нестачу з мережі й зазвичай вимикається під час аварійного відключення. Для роботи без мережі потрібні гібридний інвертор та акумулятори; їхню ємність підбирають за критичними приладами й бажаним часом автономності.';
      links = [{ label: 'Гібридні інвертори', url: 'inverters-hybrid.html' }, { label: 'Акумулятори', url: 'batteries-lifepo4.html' }];
    } else if (/теплов.*насос|електромоб|зарядк.*авто|бойлер|кондиціон|електроопален/.test(lower)) {
      answer =
        'Таке навантаження можна врахувати, але важливі не лише кВт·год, а й пікова потужність одночасно ввімкнених приладів. Напишіть потужність теплового насоса, зарядки, бойлера чи іншого обладнання та скільки годин воно працює. Для резерву менеджер окремо перевірить пускові струми й допустиму потужність інвертора.';
    } else if (/скільки.*панел.*(?:10|15|20|30|50)\s*квт|(?:10|15|20|30|50)\s*квт.*скільки.*панел/.test(lower)) {
      var requestedPowerMatch = lower.match(/(10|15|20|30|50)\s*квт/);
      var requestedPower = requestedPowerMatch ? Number(requestedPowerMatch[1]) : 0;
      var estimatedPanels = Math.ceil((requestedPower * 1.2 * 1000) / catalog.panel.watts);
      var estimatedDc = (estimatedPanels * catalog.panel.watts / 1000).toFixed(1).replace('.', ',');
      answer =
        'Для станції з інвертором ' + requestedPower + ' кВт попередній орієнтир — близько ' + estimatedPanels +
        ' панелей по 620 Вт, тобто ' + estimatedDc + ' кВт DC. Точну кількість визначають за допустимим DC/AC співвідношенням інвертора, орієнтацією, затіненням і доступною площею.';
      links = [{ label: 'Сонячні панелі', url: 'panels.html' }];
    } else if (/чому.*(longi|лонг|панел)|навіщо.*(longi|лонг)/.test(lower)) {
      answer =
      ...
      links = [{ label: 'Підібрати обладнання', url: 'configurator.html' }];
  ...
  function looksLikeConsultingTopic(text) {
    return /[?？]|привіт|добрий|вітаю|станц|спожив|електроенерг|не знаю|чому|навіщо|який|яка|які|як |чи |скільки|коли|де |що |longi|лонг|deye|дея|панел|інвертор|ціна|кошту|вартіст|гарант|наявн|склад|окуп|економ|генерац|виробіт|двосторон|bifacial|біфац|акум|батар|резерв|автоном|конструкц|земл|ґрунт|наземн|дах|покрів|монтаж|кріплен|кабел|захист|щит|фаз|достав|оплат|документ|сертиф|альтернатив|обслугов|сервіс|ніч|без сонц|похмур|взимк|теплов|електромоб|зарядк|бойлер|кондиціон|опален/.test(String(text).toLowerCase());
  }
  ...
  function handle(text) {
    ...
    if (!endpoint) {
      window.setTimeout(function () { localReply(text); }, 180);
      return;
    }
